---
title: "12.3 REITs, Digital Assets"
categories: [ Finance, Course]
tags: [Basic]
description: "REITs (Real Estate Investment Trusts) और Digital Assets"
date: 2025-08-12T08:00:00+05:30
author: ahmad
type: docs
---

**Module 12: Wealth Creation & Passive Income Sources**

**REITs (Real Estate Investment Trusts)** और **Digital Assets** दोनों का detail, खासकर passive income के नजरिए से।

---

## **1. REITs (Real Estate Investment Trusts)**

* **क्या है?**
  REITs ऐसी कंपनियां होती हैं जो **income-generating real estate** (जैसे malls, office buildings, warehouses, hotels) में invest करती हैं और उस income का बड़ा हिस्सा investors को **dividends** के रूप में देती हैं।
* **Passive Income कैसे देता है?**

  * आप REITs के शेयर खरीदते हो
  * वो properties से rent earn करते हैं
  * उस rent का हिस्सा आपको regular dividend के रूप में मिलता है
* **फायदे:**

  * Property खरीदने की जरूरत नहीं
  * Small amount से शुरू कर सकते हो
  * Highly liquid (stock market में आसानी से बेच सकते हो)
* **Example:** Embassy Office Parks REIT, Mindspace Business Parks REIT
* **Risk:**

  * Market fluctuation
  * Real estate sector slowdown

---

## **2. Digital Assets**

Digital assets वो online resources हैं जो **value generate करते हैं और income produce करते हैं**, जैसे:

### **A. Domain Names**

* Popular or brandable domain खरीदकर बाद में high price पर बेचना (Domain Flipping)
* Example: voice.com \$30 million में बिका था

### **B. NFTs (Non-Fungible Tokens)**

* Digital art, music, videos या in-game assets को blockchain पर mint करके बेचना
* Risk high है क्योंकि market volatile है

### **C. Crypto Staking / Yield Farming**

* Crypto को blockchain network पर stake करके interest (rewards) earn करना
* Example: Ethereum staking, Cardano staking
* Risk: Price volatility और platform risk

### **D. Digital Products**

* Ebooks, Templates, Stock Photos, Online Courses जो एक बार बनाकर infinite बार बेच सकते हो
* 100% passive once system setup हो जाए

---

📌 **Comparison Table: REITs vs Digital Assets**

| Feature            | REITs                    | Digital Assets                             |
| ------------------ | ------------------------ | ------------------------------------------ |
| Initial Investment | Low to Medium            | Low to High (asset type पर depend करता है) |
| Risk Level         | Moderate                 | Low to Very High                           |
| Liquidity          | High (Stock Exchange)    | Medium to High (market पर depend)          |
| Passive Potential  | High (Regular dividends) | High (but uncertain for volatile assets)   |
| Effort             | Minimal                  | Setup & marketing needed                   |