...
If we talk about Velocity has a robust architecture that focuses on cloud native elasticity, resiliency security and scale. it is built from the ground up on microservices-oriented architecture

Velocity has composed a series of fit to purpose experiences serving a wide range of personas including employers, participants provider, broker and investors
...
An extensive library of API come pre packaged with Velocity which streamlines interoperability by using Restful interfaces and GraphQL queries, kafka based queue for message based integrations and data lake for real time streaming large data processing and analytics

Velocity's out of the box medium grained RESTful APIs are stateless support adaptable JSON payloads operate under Oath2 token based authentication and support versioning and API gateways

Velocity is built natively on AWS

A Modern Technology Stack
Velocity employs a multi layered technology stack spanning user interfaces, business applications, on demand elasticity and managed services all of which are deployed natively on AWS cloud
...
Velocity deliver on demand compute and storage elasticity using various AWS cloud services like Elastic Beanstalk, Lambda S3 etc

The Velocity manage services layer represent a turn-key offering that leverages the resiliency scalability and security fetuses.
